...
async def test_tool_execution(manager):
    ...
    # 执行工具
    params = {"key": "value"}
    await manager.execute_tool("tool1", params)
...
async def test_tool_with_alias(manager):
    ...
    # 执行别名工具
    await manager.execute_tool("aliased_tool", {})
...
async def test_dynamic_server_management(manager):
    ...
    await manager.add_or_update_server(new_server)

    # 验证新服务器启动
    assert "http_server" not in manager.active_clients
    await manager.start_client("http_server")
    ...
    await manager.add_or_update_server(updated_server)
    ...
    # 移除服务器
    await manager.remove_server("http_server")
    assert "http_server" not in manager.active_clients
...
async def test_auto_reconnect_disabled(manager):
    ...
    with pytest.raises(RuntimeError):
        await manager.add_or_update_server(updated_config)
...
async def test_get_available_tools(manager):
    ...
    # 获取工具
    tools = []
    async for tool in manager.get_available_tools():
        tools.append(tool)
...
async def test_error_handling(manager):
    ...
    await manager.add_or_update_server(bad_server)
    ...
    with pytest.raises(TimeoutError):
        await manager.execute_tool("tool1", {}, timeout=0.1)
...
async def test_meta_data_injection(manager):
    ...
    # 执行工具
    result = await manager.execute_tool("tool1", {})
