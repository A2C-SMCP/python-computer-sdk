...
async def organize_desktop(
    *,
    windows: list[tuple[SERVER_NAME, Resource]],
    size: int | None,
    ...
    grouped: dict[SERVER_NAME, list[tuple[Resource, int, bool, int]]] = {}
    for idx, (server, res) in enumerate(windows):
        try:
            ...
            # 非法 URI 或解析失败的资源，跳过
            continue
        grouped.setdefault(server, []).append((res, prio, fullscreen, idx))
    ...
    for srv in server_order:
        ...
        fullscreen_items = [(res, prio, fs, idx) for (res, prio, fs, idx) in items if fs]
        if fullscreen_items:
            res, _prio, _fs, _idx = min(fullscreen_items, key=lambda x: x[3])
            result.append(str(res.uri))
            continue  # 该服务器仅加入这一个，转下一个服务器

        # 否则按优先级加入多条直到 cap
        for res, _prio, _fs, _idx in items:
            if len(result) >= cap:
                break
            result.append(str(res.uri))
